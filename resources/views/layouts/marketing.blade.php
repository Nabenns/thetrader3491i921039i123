...
<head>
    ...
    <style>
        ...
        .animate-marquee {
            /* Slower duration because the width is now much longer (20 repetitions) */
    ...
    <div class="fixed top-0 z-[60] w-full bg-background-dark/80 backdrop-blur-md border-b border-white/5 h-12 flex items-center overflow-hidden"
        x-data="{
                 prices: [
                     ...
                     { symbol: 'XAU', price: '...', change: 0 },
                 ],
                 async fetchPrices() {
                     try {
                         // Fetch from CoinGecko (Verified working)
                         ...
                         const map = [
                             ...
                             { id: 'pax-gold', symbol: 'XAU' }
                         ];

                         this.prices = map.map(item => {
                             const coin = data[item.id];
                             return {
                                 symbol: item.symbol,
                                 price: coin ? parseFloat(coin.usd).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00',
                                 change: coin ? parseFloat(coin.usd_24h_change).toFixed(2) : 0
                             };
                         });
                     } catch (e) {
                         console.error('Ticker error, using fallback:', e);
                         // Fallback data if API fails
                         this.prices = [
                             { symbol: 'BTC', price: '96,432.00', change: 2.5 },
                             { symbol: 'ETH', price: '2,740.00', change: -1.2 },
                             { symbol: 'XAU', price: '2,650.00', change: 0.5 }
                         ];
                     }
                 },
                 init() {
                     this.fetchPrices();
                     // Refresh every 30s to respect CoinGecko free tier rate limits
        ...
        <div class="flex animate-marquee whitespace-nowrap">
            <!-- Loop 20 times for truly seamless infinite scroll even on wide screens -->
    ...
    <div class="min-h-screen flex flex-col pt-12"> <!-- Added pt-12 to account for ticker height -->
        ...
        <nav x-data="{ scrolled: false, mobileMenuOpen: false }" @scroll.window="scrolled = (window.pageYOffset > 20)"
                ...
                <div class="flex justify-between items-center h-16">
                    ...
                    <div class="flex-shrink-0 flex items-center">
                        <a href="/" class="text-2xl font-bold group flex items-center gap-2">
                            <div
                                class="w-8 h-8 flex items-center justify-center text-white group-hover:rotate-12 transition-transform duration-300 overflow-hidden">
                    ...
                    <div class="hidden md:flex items-center space-x-8">
                        ...
                        @auth
                            <a href="{{ url('/dashboard') }}"
                                class="text-gray-300 hover:text-primary transition font-medium">Dashboard</a>
                        @else
                            <a href="{{ route('login') }}"
                                class="text-gray-300 hover:text-primary transition font-medium">Masuk</a>
                            <a href="{{ route('register') }}"
                                class="bg-primary hover:bg-secondary text-white px-5 py-2.5 rounded-xl font-semibold transition shadow-lg shadow-primary/20 hover:shadow-primary/40 hover:-translate-y-0.5">
                                Daftar Sekarang
                            </a>
                        @endauth
                    </div>
            ...
            <div x-show="mobileMenuOpen" x-transition:enter="transition ease-out duration-200"
                ...
                class="md:hidden glass bg-background-dark/90 border-t border-white/10 absolute w-full" x-cloak>
                <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                    <a href="#features"
                        class="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:text-white hover:bg-white/10">Fitur</a>
                    <a href="#pricing"
                        class="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:text-white hover:bg-white/10">Harga</a>
                    @auth
                        <a href="{{ url('/dashboard') }}"
                            class="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:text-white hover:bg-white/10">Dashboard</a>
                    @else
                        <a href="{{ route('login') }}"
                            class="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:text-white hover:bg-white/10">Masuk</a>
                        <a href="{{ route('register') }}"
                            class="block w-full text-center mt-4 bg-primary hover:bg-secondary text-white px-5 py-3 rounded-xl font-bold transition">
                            Daftar Sekarang
                        </a>
                    @endauth
                </div>
        ...
        <footer
            class="fixed bottom-0 left-0 w-full h-[300px] -z-10 bg-background-dark border-t border-white/5 flex items-center justify-center">
